...
As a keystone of the cryptocurrency ecosystem, democracy has been severely threatened by the monopolization of mining farms in recent years. Existing countermeasures by enforcing new mining algorithms can effectively address this, but on the other hand have rendered web cryptomining unprofitable and constantly fading.

To address this, we design and implement an open-source web miner dubbed PropaMiner to make web cryptomining efficient and profitable, thus reviving the democracy of the cryptocurrency ecosystem. PropaMiner employs a novel methodology of symbolic-constrained migratory emulation, which discovers and exploits the pervasive isomorphic propagations of vector elements in the mining algorithms for instruction merging and acceleration.

This repository contains the source code of PropaMiner and the data involved in this work.

**Note that all measurements and analyses throughout this work are conducted under a well-established IRB, and no personally identifiable information is collected.**
...
WebRandomX is a high-performance web miner. It implements the key mechanisms of our symbolic-constrained migratory emulation for RandomX. The core of WebRandomX is built into a WASM binary so that one can load it and perform web mining after the page is loaded from the remote server. We provide an example web page to bootstrap the binary. 
...
* **Cloud server** (for building and hosting PropaMiner): at least 1 GB memory and 2 Gbps bandwidth
...
We provide our measurement data regarding the tested WRVM programs at [WebCryptomining.github.io/dataset](https://github.com/WebCryptomining/WebCryptomining.github.io/tree/main/dataset)
